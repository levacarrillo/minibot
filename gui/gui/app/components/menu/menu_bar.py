...
class MenuBar:
    def __init__(self, context):
        ...
        submenu_canvas.add_command(label = " 500 x 500 ",
                                    command = lambda : context.canvas_panel_resize(500, 500))
        submenu_canvas.add_command(label = " 600 x 600 ",
                                    command = lambda : context.canvas_panel_resize(600, 600))
        submenu_canvas.add_command(label = " 700 x 700 ",
                                    command = lambda : context.canvas_panel_resize(700, 700))
        submenu_canvas.add_command(label = " 800 x 800 ",
                                    command = lambda : context.canvas_panel_resize(800, 800))
